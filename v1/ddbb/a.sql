...
CREATE DATABASE IF NOT EXISTS `deaw` DEFAULT CHARACTER SET utf8 COLLATE utf8_spanish_ci;
USE `deaw`;

DROP TABLE IF EXISTS `deaw`.`stock`;
DROP TABLE IF EXISTS `deaw`.`producto`;
DROP TABLE IF EXISTS `deaw`.`familia`;
DROP TABLE IF EXISTS `deaw`.`tienda`;


-- Creamos las tablas
CREATE TABLE `deaw`.`tienda` (
`cod` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
`nombre` VARCHAR( 100 ) NOT NULL ,
`tlf` VARCHAR( 13 ) NULL
) ENGINE = INNODB;

CREATE TABLE `deaw`.`producto` (
...
CREATE TABLE `deaw`.`familia` (
`cod` VARCHAR( 6 ) NOT NULL ,
`nombre` VARCHAR( 200 ) NOT NULL ,
PRIMARY KEY ( `cod` )
) ENGINE = INNODB;

CREATE TABLE `deaw`.`stock` (
